-- ============================================
-- FIX CORE COLUMNS CAMPANHAS
-- Migração 036: Garante colunas fundamentais em campanhas
-- ============================================

DO $$
BEGIN
    -- Verificar colunas FUNDAMENTAIS
    -- Titulo (Core)
    BEGIN 
        ALTER TABLE campanhas ADD COLUMN titulo VARCHAR(255); 
    EXCEPTION 
        WHEN duplicate_column THEN 
            RAISE NOTICE 'Coluna titulo já existe';
    END;
    ...
    -- Orçamento
    BEGIN 
        ALTER TABLE campanhas ADD COLUMN orcamento DECIMAL(10,2); 
    EXCEPTION 
        WHEN duplicate_column THEN 
            RAISE NOTICE 'Coluna orcamento já existe';
    END;

    -- Datas
    BEGIN 
        ALTER TABLE campanhas ADD COLUMN data_inicio DATE; 
    EXCEPTION 
        WHEN duplicate_column THEN 
    ...
    BEGIN 
        ALTER TABLE campanhas ADD COLUMN data_fim DATE; 
    EXCEPTION 
        WHEN duplicate_column THEN 
    ...
    -- Status
    BEGIN 
        ALTER TABLE campanhas ADD COLUMN status VARCHAR(50); 
    EXCEPTION 
        WHEN duplicate_column THEN 
    ...
    IF NOT EXISTS (
        ...
        WHERE table_name = 'campanhas' AND column_name = 'empresa_id'
    ) THEN
        -- Isso seria catastrófico, mas vamos checar
        RAISE EXCEPTION 'Coluna empresa_id não existe! Tabela pode estar corrompida.';
    END IF;

END $$;

-- Recriar função para garantir que ela use o cache de schema atualizado
