-- ============================================
-- FIX RASCUNHO DEFINITIVO
-- Migração 035: Garante colunas de rascunho e atualiza função RPC
-- ============================================

-- 1. GARANTIR TODAS AS COLUNAS EM CAMPANHAS
-- Mesmo que 020 ou 029 tenham sido rodados, verificamos novamente
DO $$
BEGIN
    -- Geolocalização
    BEGIN ALTER TABLE campanhas ADD COLUMN localizacao_tipo VARCHAR(50); EXCEPTION WHEN duplicate_column THEN END;
    BEGIN ALTER TABLE campanhas ADD COLUMN raio_km DECIMAL(10,2); EXCEPTION WHEN duplicate_column THEN END;
    BEGIN ALTER TABLE campanhas ADD COLUMN centro_latitude DECIMAL(10,8); EXCEPTION WHEN duplicate_column THEN END;
    BEGIN ALTER TABLE campanhas ADD COLUMN centro_longitude DECIMAL(11,8); EXCEPTION WHEN duplicate_column THEN END;
    BEGIN ALTER TABLE campanhas ADD COLUMN poligono_coordenadas JSONB; EXCEPTION WHEN duplicate_column THEN END;
    BEGIN ALTER TABLE campanhas ADD COLUMN cidades TEXT[]; EXCEPTION WHEN duplicate_column THEN END;
    BEGIN ALTER TABLE campanhas ADD COLUMN estados TEXT[]; EXCEPTION WHEN duplicate_column THEN END;
    BEGIN ALTER TABLE campanhas ADD COLUMN regioes TEXT[]; EXCEPTION WHEN duplicate_column THEN END;
    BEGIN ALTER TABLE campanhas ADD COLUMN areas_especificas JSONB; EXCEPTION WHEN duplicate_column THEN END;
    BEGIN ALTER TABLE campanhas ADD COLUMN excluir_areas JSONB; EXCEPTION WHEN duplicate_column THEN END;

    -- Nicho e Categorias
    BEGIN ALTER TABLE campanhas ADD COLUMN nicho VARCHAR(100); EXCEPTION WHEN duplicate_column THEN END;
    BEGIN ALTER TABLE campanhas ADD COLUMN categorias TEXT[]; EXCEPTION WHEN duplicate_column THEN END;

    -- Público Alvo
    BEGIN ALTER TABLE campanhas ADD COLUMN publico_alvo JSONB; EXCEPTION WHEN duplicate_column THEN END;
    BEGIN ALTER TABLE campanhas ADD COLUMN horarios_exibicao JSONB; EXCEPTION WHEN duplicate_column THEN END;
    BEGIN ALTER TABLE campanhas ADD COLUMN dias_semana INTEGER[]; EXCEPTION WHEN duplicate_column THEN END;

    -- Objetivos
    BEGIN ALTER TABLE campanhas ADD COLUMN objetivo_principal VARCHAR(50); EXCEPTION WHEN duplicate_column THEN END;
    BEGIN ALTER TABLE campanhas ADD COLUMN objetivos_secundarios TEXT[]; EXCEPTION WHEN duplicate_column THEN END;
    BEGIN ALTER TABLE campanhas ADD COLUMN kpis_meta JSONB; EXCEPTION WHEN duplicate_column THEN END;
    BEGIN ALTER TABLE campanhas ADD COLUMN estrategia VARCHAR(100); EXCEPTION WHEN duplicate_column THEN END;

    -- Controle de Rascunho
    BEGIN ALTER TABLE campanhas ADD COLUMN is_rascunho BOOLEAN DEFAULT FALSE; EXCEPTION WHEN duplicate_column THEN END;
    BEGIN ALTER TABLE campanhas ADD COLUMN saldo_insuficiente BOOLEAN DEFAULT FALSE; EXCEPTION WHEN duplicate_column THEN END;
    BEGIN ALTER TABLE campanhas ADD COLUMN rascunho_salvo_em TIMESTAMP WITH TIME ZONE; EXCEPTION WHEN duplicate_column THEN END;
END $$;


-- 2. RECRIAR FUNÇÃO salvar_rascunho_campanha
-- Garantimos que a função usa exatamente as colunas que existem
...
GRANT EXECUTE ON FUNCTION salvar_rascunho_campanha TO authenticated;
