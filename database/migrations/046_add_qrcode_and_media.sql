-- ============================================
-- ADD QR CODE AND MEDIA COLUMNS
-- Migração 046: Adiciona suporte a Link QR Code e Mídias (URLs)
-- ============================================

-- 1. ADICIONAR COLUNAS NA TABELA CAMPANHAS
DO $$
BEGIN
    -- Link do QR Code (URL de destino)
    BEGIN 
        ALTER TABLE campanhas ADD COLUMN qr_code_link TEXT; 
    EXCEPTION 
        WHEN duplicate_column THEN NULL; 
    END;

    -- URLs de Mídias (Array/JSON para rascunho simplificado)
    -- Ex: ["https://bucket.../img1.jpg", "https://bucket.../video1.mp4"]
    BEGIN 
        ALTER TABLE campanhas ADD COLUMN midias_urls JSONB; 
    EXCEPTION 
        WHEN duplicate_column THEN NULL; 
    END;
END $$;

-- 2. ATUALIZAR RPC SALVAR RASCUNHO
-- Inclui os novos campos na lógica de update/insert
...
BEGIN
  ...
  IF p_campanha_id IS NOT NULL AND EXISTS (
    ...
    SET 
      ...
      -- Novos Campos (QR Code e Mídias)
      qr_code_link = COALESCE((p_dados->>'qr_code_link')::TEXT, qr_code_link),
      midias_urls = COALESCE((p_dados->>'midias_urls')::JSONB, midias_urls),
  ...
  ELSE
    ...
    VALUES (
      ...
      -- Valores novos campos
      CASE WHEN p_dados ? 'qr_code_link' AND p_dados->>'qr_code_link' != '' THEN p_dados->>'qr_code_link' ELSE NULL END,
      CASE WHEN p_dados ? 'midias_urls' AND p_dados->'midias_urls' IS NOT NULL AND p_dados->'midias_urls'::text != 'null' THEN p_dados->'midias_urls' ELSE NULL END,
